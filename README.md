...
So the agent can apply a plugin update you name, but cannot discover that one is available - on its own. If you also run [unraid-compose-gateway](https://github.com/shanelord01/unraid-compose-gateway), its `ucg_plugin_updates` tool fills exactly this gap: it replicates the same local-`.plg`-vs-remote-`.plg` version check Unraid's own web UI does, and reports which installed plugins have an update available. Detection and application stay two separate steps either way - this repo applies, that one detects.

### `unraid_install_plugin` reporting success is not proof the update applied
Confirmed live, 2026-08-19, on a plugin *other than* `dynamix.unraid.net` (that one has its own, separate, well-understood failure mode below): the GraphQL API logged the install as started and completed successfully seconds apart - no error, no crash - but the installed version had not actually changed until the operator separately used the Unraid webGUI. Root cause not understood; not known whether this affects every plugin or only some.

Always re-check after calling this tool rather than trusting its own response - if you also run [unraid-compose-gateway](https://github.com/shanelord01/unraid-compose-gateway), call `ucg_plugin_updates` with `force=true` and confirm `update_available` is now false before reporting a plugin as updated. A plugin still showing as needing an update after the attempt should be reported as needing a manual webGUI update, not retried.

### Cannot update dynamix.unraid.net (Unraid Connect) itself
This is a distinct, well-understood failure mode, not the same as the above. Confirmed live, not theoretical: `unraid_install_plugin` goes through this same GraphQL API, and `dynamix.unraid.net` is the plugin that *runs* that API service. Installing an update to it kills the API process mid-install - `graphql-api.log` showed the process exiting on signal 130 partway through, then rolling back cleanly to the prior version on restart. No corruption, but the update never actually applies, no matter how many times it's retried.
